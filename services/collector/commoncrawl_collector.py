...
class CommonCrawlCollector:
    ...
    def __init__(self):
        ...
        self.economic_keywords = [
            'colombia economía', 'colombia economy', 'colombia bolsa', 'colombia stock',
            'colombia finanzas', 'colombia finance', 'colcap', 'BVC colombia',
            'colombia peso', 'colombia banco', 'bancolombia', 'banco bogotá',
            'ecopetrol', 'avianca', 'colombia petróleo', 'colombia oil',
            'colombia inflación', 'colombia inflation', 'colombia PIB', 'colombia GDP',
            'colombia exportación', 'colombia export', 'colombia comercio',
            'colombia TRM', 'colombia dólar', 'colombia dollar'
        ]
    ...
    def _get_recent_indices(self) -> List[str]:
        ...
        known_indices = [
            "2024-51", "2024-46", "2024-42", "2024-38", "2024-33",  # 2024
            "2024-30", "2024-26", "2024-22", "2024-18", "2024-10",
            "2025-04",  # 2025 (si está disponible)
        ]
        
        indices = [f"CC-MAIN-{idx}" for idx in known_indices]
        
        logger.info(f"Índices a consultar: {', '.join(indices[:3])}... (total: {len(indices)})")
        return indices
    ...
    def search_commoncrawl(self, keyword: str, index: str, max_results: int = 100) -> List[Dict]:
        ...
        try:
            ...
            query_url = f"{cdx_url}?url=*.co/*&matchType=domain&filter=status:200&output=json"
    ...
    def _is_relevant_url(self, url: str, keyword: str) -> bool:
        ...
        news_domains = ['eltiempo', 'semana', 'portafolio', 'larepublica', 
                       'dinero', 'elespectador', 'bloomberg', 'reuters', 
                       'wsj', 'ft.com', 'eleconomista']
        ...
        contains_keyword = any(part in url_lower for part in keyword_parts if len(part) > 3)
        ...
        try:
            ...
            if response.status_code in [200, 206]:
                ...
                if html_start > 0:
                    ...
                    title_text = title.get_text().strip() if title else warc_record['url']
                    
                    # Extraer fecha de publicación (intentar varios métodos)
                    pub_date = self._extract_publish_date(soup, warc_record['timestamp'])
            ...
            logger.debug(f"Error obteniendo contenido de {warc_record.get('url', 'unknown')}: {str(e)}")
    ...
    def collect_from_indices(self, days_back: int = 365) -> List[Dict]:
        ...
        logger.info(f"🔍 Iniciando recolección de Common Crawl (últimos {days_back} días)")
        ...
        for index_idx, index in enumerate(indices_to_search, 1):
            logger.info(f"📥 Procesando índice {index_idx}/{len(indices_to_search)}: {index}")
            
            for keyword_idx, keyword in enumerate(self.economic_keywords[:20], 1):  # Primeros 20 keywords
                try:
                    # Buscar artículos para este keyword
                    warc_records = self.search_commoncrawl(keyword, index, max_results=50)
                    
                    logger.info(f"  [{keyword_idx}/20] '{keyword}': {len(warc_records)} encontrados")
            ...
            logger.info(f"✅ Índice {index} completado. Total acumulado: {len(all_articles)}")
