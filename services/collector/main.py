...
class GDELTCollector:
    ...
    def __init__(self):
        ...
        self.economic_keywords = [
            ...
            "grupo aval",
            "colombia crédito",
            "colombia tasa interés",
            "colombia banrep",
            "banco república colombia",
            ...
            "colombia déficit",
            "colombia deuda",
            "colombia presupuesto",
            "colombia balanza comercial",
            # Ciudades económicas
            "bogotá economía",
            "medellín economía",
            "cali economía",
            "barranquilla economía",
            "cartagena economía",
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if "articles" in data:
                        ...
                        successful += 1
                        logger.info(
                            f"  [{i}/{len(self.economic_keywords)}] '{keyword}': {count} ✓"
                        )
    ...
    def save_to_database(self, articles: List[Dict]):
        ...
        try:
            ...
            for i in range(0, len(batch_data), BATCH_SIZE):
                ...
                logger.info(
                    f"  📦 Batch {i // BATCH_SIZE + 1}: {cursor.rowcount} insertados"
                )

            conn.commit()
            logger.info(
                f"💾 BATCH INSERT completado: {saved}/{len(batch_data)} guardados"
            )
    ...
    def collect(self):
        ...
        logger.info(
            f"✅ Completado en {int(elapsed)}s - {len(articles)} artículos, {saved} guardados"
        )
